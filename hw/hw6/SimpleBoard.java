...
public class SimpleBoard {
  ...
  public boolean equals(Object board) {
    // if "board" is not a SimpleBoard, return false (rather than throwing a ClassCastException)
    if(!(board instanceof SimpleBoard)) return false;
    // check if the same position of board and "this" object have the same value.
    SimpleBoard oneBoard = (SimpleBoard) board;
    for(int x = 0; x < DIMENSION; x++) {
      for(int y = 0; y < DIMENSION; y++) {
        if(oneBoard.grid[x][y] != grid[x][y]) return false;
      }
    }
    return true;
  }
  ...
  public int hashCode() {
    ...
    for(int x = 0; x < DIMENSION; x++) {
      for(int y = 0; y < DIMENSION; y++) {
        // think of each cell as a digit of a base-3 number (with 64 digits)
        // and covert the number to a single int.
        code = grid[x][y] + 3 * code;
      }
    }
    return (int)code;
  }
