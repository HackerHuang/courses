...
public class SList {
  ...
  public void squish() {
    ...
    // traversal beginning from the second node (if exist)
    for(SListNode node = head.next; node != null; node = node.next) {
      if(!node.item.equals(currentNode.item)) {
        currentNode.next = node;
        currentNode = currentNode.next;
      } else {
        size--; //corresponds to the beginning of traversal(from 2nd node)
      }
    }
    // set the next node after currentNode to null (don't forget!)
    currentNode.next = null;
    //check if size is correct
    assert size == countLength(): "wrong size...current size is" + size + " correct size is " + countLength();
    /*
    SListNode current = head;
    for(SListNode previous = head; previous != null; previous = previous.next) {
      while (current != null) {
        if (current.item.equals(previous.item)) {
  ...
  public void twin() {
    ...
    for(SListNode currentNode = head; currentNode != null; ) {
  ...
  private int countLength() {
    int count = 0;
    for(SListNode node = head; node != null; node = node.next) {
      count++;
    }
    return count;
  }
