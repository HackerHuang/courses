...
public class Sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    ...
    final int base = 16;
    final int baseDigits = 4;
    final int maxBaseValue = 15;
    int[] sortedKeys = new int[keys.length];
    int[] counts = new int[base];
    // step 1: counting the frequency of each value of the base-16 digits.
    for(int key : keys){
      int value = (key >> baseDigits * whichDigit) & maxBaseValue;
      counts[value]++;
    }
...
//    for(int cnt : counts){
//      int tmp = cnt;
//      cnt = cumSum;// NOTE: cnt change?
//      cumSum += tmp;
//    }
    for(int i = 0; i < counts.length; i++){
      int tmp = counts[i];
      counts[i] = cumSum;// NOTE: cnt change?
      cumSum += tmp;
    }
    // step 3: sort the keys
    for(int key : keys){
      int baseValue = (key >> baseDigits * whichDigit) & maxBaseValue;
      sortedKeys[counts[baseValue]++] = key;
    }
    return sortedKeys;
  }
  ...
  public static int[] radixSort(int[] keys) {
    ...
    int[] beforeSort = keys.clone();//make a new copy of keys.
    int[] afterSort = beforeSort;
    for(int digit = 0; digit <= maxDigit; digit++){
      afterSort = countingSort(beforeSort, digit);
      beforeSort = afterSort;
      printEachPass(afterSort, digit);
    }
    return afterSort;
  }

  private static void printEachPass(int[] afterSort, int digit){
...
//    System.out.println("\n");
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Before radix sort, the keys are: ");
    yell(keys);
//    keys = countingSort(keys, 1);
    keys = radixSort(keys);
    System.out.println("\nAfter radix sort, the keys are: ");
    yell(keys);
  }
