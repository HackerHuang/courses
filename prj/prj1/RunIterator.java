...
public class RunIterator implements Iterator {
  ...
  RunNode head;
  RunNode currentNode;
  boolean isVisited; // if "sentinel" has been visited, used in hasNext() & next().
//  int size;
...
//    size = lst.length();
  ...
  public boolean hasNext() {
    ...
    return (!isVisited) || (currentNode != head);
  }
  ...
  public int[] next() {
    ...
    // determine if "sentinel" is visited.
    if(!isVisited) isVisited = true;
    int[] currentRun = new int[] {currentNode.len, currentNode.red,
                                  currentNode.green, currentNode.blue};
