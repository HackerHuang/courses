...
public class SplayTree implements Dictionary {
    ...
    private void zigZag(BinaryTreeNode node) {
        assert node != null && node.parent != null && node.parent.parent != null :
                "ERROR: node OR node's parent OR node's grandparent is EMPTY.";
        assert (node.parent.leftChild == node && node.parent.parent.rightChild == node.parent)
                || (node.parent.rightChild == node && node.parent.parent.leftChild == node.parent) :
                "ERROR: node, node's parent & grandparent does NOT satisfy ZIGZAG structure.";
    ...
    private void zigZig(BinaryTreeNode node) {
        ...
        assert node != null && node.parent != null && node.parent.parent != null :
                "ERROR: node OR node.parent OR node.parent.parent is EMPTY.";
        assert (node.parent.leftChild == node && node.parent.parent.leftChild == node.parent)
                || (node.parent.rightChild == node && node.parent.parent.rightChild == node.parent):
                "ERROR: node, node's parent & grandparent does NOT form ZIGZIG.";

        if(node.parent.leftChild == node){
            // REMEMBER to first rotate node's parent, then rotate itself.
        ...
        while (node.parent != null) {
            ...
            if(grandParent != null){
                ...
                if((node == parent.leftChild && parent == grandParent.rightChild)
                        || (node == parent.rightChild && parent == grandParent.leftChild)){
                    zigZag(node);
                } else {
                    // CASE 2: zig-zig
                    zigZig(node);
    ...
    /*
    // non-recursive version by Frank.
    private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node){
        BinaryTreeNode toBeSplayed = findWithoutSplay(key, node);
        if(key.compareTo(toBeSplayed.entry.key) == 0) {
            splayNode(toBeSplayed);
            return toBeSplayed;
        } else {
            return null;
        }
    ...
    */
    ...
    private BinaryTreeNode findWithoutSplay(Comparable key, BinaryTreeNode node){
    ...
    public Entry remove(Object key) {
        if(root == null) {
            System.err.println("Invalid operation: the splay tree is empty.");
        }
        Comparable compKey = (Comparable)key;
        BinaryTreeNode toBeRemoved = findWithoutSplay(compKey, root);
        if(compKey.compareTo(toBeRemoved.entry.key) != 0) return null;// entry not found.

        Entry removedEntry = toBeRemoved.entry;

        if(toBeRemoved.rightChild != null && toBeRemoved.leftChild != null){
            BinaryTreeNode toReplace = getMinInTree(toBeRemoved.rightChild);
            toBeRemoved.entry = toReplace.entry;
            removeAndSplay(toReplace);
        } else {
            removeAndSplay(toBeRemoved);
        }
    ...
    private void removeAndSplay(BinaryTreeNode node){
        assert !(node.leftChild != null && node.rightChild != null) : "ERROR: failed operation: node NOT removed.";
        assert node.parent.leftChild == node || node.parent.rightChild == node : "ERROR: invalid node.";

        BinaryTreeNode child = (node.leftChild != null ? node.leftChild: node.rightChild);
        BinaryTreeNode exParent = node.parent;

        if(exParent.leftChild == node){
            exParent.leftChild = child;
            if(child != null) child.parent = exParent;
        } else {
            exParent.rightChild = child;
            if(child != null) child.parent = exParent;
        }
    ...
    private BinaryTreeNode getMinInTree(BinaryTreeNode node) {
            // find the minimum-key node in tree rooted at "node".
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nPART IV:  Testing find()");
        System.out.println("\nSome find() operations on a new tree to test splayNode():\n");
        System.out.println("Inserting 3A, 7B, 4C, 2D, 9E, 1F");
        SplayTree tree3 = new SplayTree();
        tree3.insert(new Integer(3), "A");
        tree3.insert(new Integer(7), "B");
        tree3.insert(new Integer(4), "C");
        tree3.insert(new Integer(2), "D");
        tree3.insert(new Integer(9), "E");
        tree3.insert(new Integer(1), "F");
        System.out.println("Tree 2 is:  " + tree2);
        shouldBe = "1F((2D(3A((4C)7B)))9E)";
        if (!tree3.toString().equals(shouldBe)) {
            System.err.println("  ERROR:  SHOULD BE " + shouldBe);
        }
        tree3.testFind(7, "B");
        System.out.println("Tree 2 is:  " + tree2);
        shouldBe = "(1F((2D)3A(4C)))7B(9E)";
