...
//    UDGraph newGraph = new UDGraph(vertices);
  ...
  /*
  // O(n^2 + n*max{degree(v)*degree(u)}).
  public UDGraph length2Paths(){
    UDGraph newGraph = new UDGraph(vertices);
    // Put your answer to Part I here.
    int[][] incidentEdges = getIncidentEdges();
    for(int u = 0; u < vertices; ++u){
      for(int v : incidentEdges[u]){
        for(int w : incidentEdges[v]){
          // WARNING: if one edge already EXIST, no need to add it and increment "edges".
          newGraph.addEdge(u, w);
//          newGraph.adjMatrix[u][w] = true;
//          newGraph.edges++;
        }
      }
    }
  ...
  */
  private int[][] getIncidentEdges() {
    int[] numOfIncidentEdges = new int[vertices];
    int[][] incidentEdges = new int[vertices][];
    for(int i = 0; i < vertices; ++i){
      for(int j = 0; j < vertices; ++j)
        if(adjMatrix[i][j]) numOfIncidentEdges[i]++;
    }
    for(int i = 0; i < vertices; ++i){
      incidentEdges[i] = new int[numOfIncidentEdges[i]];
      int k = 0;
      for(int j = 0; j < vertices; ++j)
        if(adjMatrix[i][j]) incidentEdges[i][k++] = j;
      assert k == numOfIncidentEdges[i] : "ERROR: number of incident edges NOT match.";
    }
  ...
  public UDGraph paths(int length){
    ...
    for(int origin = 0; origin < vertices; origin++){
      ...
      q.addLast(new Vertex(origin, 0));
      //isVisited[origin] = true;
      while(!q.isEmpty()){
        Vertex oneVertex = q.pollFirst();
        // CONDITION: if N = 2 level is NOT reached.
        // WARNING: In the original version: Firstly define a int[] level = new int[vertices];
        // And set if condition as "level[oneVertex] != length", which is WRONG.
        // Because "0" is updated after "1"(level 4) whereas subsequent "0"(level 4) can NOT lead to "8".
        // In this sense, newGraph.adjMatrix[8][8] == false, which is NOT correct.
        // THE CORE PROBLEM: the same vertex(with DIFFERENT levels) CAN exist in the queue at the SAME time.
        if(oneVertex.level != length) {
          for (int adjVertex = 0; adjVertex < vertices; adjVertex++) {
            if (adjMatrix[oneVertex.value][adjVertex]) {
              q.addLast(new Vertex(adjVertex, oneVertex.level+1));
            }
          }
        } else newGraph.addEdge(origin, oneVertex.value);
      }
    }
    return newGraph;
  }
  ...
  public UDGraph paths(int length) {
    ...
    int[][] tmpIncidentEdges = tmpGraph.getIncidentEdges();
    int[][] incidentEdges = getIncidentEdges();
    for(int u = 0; u < vertices; u++){
      for(int v : tmpIncidentEdges[u]){
        for(int w : incidentEdges[v]){
          // WARNING: if one edge already EXIST, no need to add it and increment "edges".
          newGraph.addEdge(u, w);
//          newGraph.adjMatrix[u][w] = true;
//          newGraph.edges++;
    ...
    int cntEdges = newGraph.countEdges();
    assert cntEdges == newGraph.edges: "ERROR: EDGE NUMBER NOT MATCH." + cntEdges + " " + newGraph.edges;
    return newGraph;
  }
*/
