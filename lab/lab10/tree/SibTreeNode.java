...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(!isValidNode()) throw new InvalidNodeException();
    SibTreeNode currentNode = firstChild;
    SibTreeNode prevNode = null;
    while(currentNode != null && c > 1){
    ...
    if(prevNode != null) {
      prevNode.nextSibling = tmp;
    } else {
      firstChild = tmp;
    }
    tmp.nextSibling = currentNode;
    tmp.parent = this;
    myTree.size++;
  }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (this != myTree.root) {
      ...
      if (this == parent.firstChild) {
        ...
        // "this" is not firstChild, find its previous sibling
        SibTreeNode node = parent.firstChild;
        while (node != null) {
          if (node.nextSibling == this) break;
        }
    ...
    // invalidate "this" and reduce size
    valid = false;
    myTree.size--;
    return;
  }
