...
public class BinaryTree implements Dictionary {
  ...
  public Entry remove(Object key) {
    ...
    if(node.leftChild == null && node.rightChild == null){
      ...
      if(node.leftChild != null && node.rightChild == null){
        replaceMe(node, node.leftChild);
      } else if(node.rightChild != null && node.leftChild == null){
        replaceMe(node, node.rightChild);
      } else {
        // CASE 3: has TWO children
        // find the smallest element in the RIGHT subtree
        BinaryTreeNode tmp = findSmallest(node.rightChild);
        replaceMe(tmp, tmp.rightChild);
  ...
  private BinaryTreeNode findSmallest(BinaryTreeNode node){
    ...
    else return findSmallest(node.leftChild);
  }

  private void replaceMe(BinaryTreeNode node, BinaryTreeNode newNode){
    assert node == null: "ERROR: null node passed in...";
    // replace ROOT (has ONE child)
    if(node.parent == null) {
      root = newNode;
      return;
    }
