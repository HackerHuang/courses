...
public class DList2 {
  ...
  private int countLength() {
    int count = 0;
    // traverse excluding the sentinel node
    // termination condition: node == head,
    // which means getting back to the starting point after a loop
    for(DListNode2 node = head.next; node != head; node = node.next) count++;
    return count;
  }
  /**
